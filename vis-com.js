...
var io = require("socket.io").listen(SOCKET_PORT, { log: false });
...
var chat = io.of("/communication/").on("connection", function (socket) {
	...
	socket.on("send-group-message", function(data){
		...
		if( validator.isNumeric(groupIndex) ){
			...
			for(var i=0; i<sockets.length; i++){
				...
				sockets[i].emit("draw-this", {drawingInstruction: data.drawingInstruction});
			}
		}else{
	...
	socket.on("group-update-layers", function(data){
		...
		if( validator.isNumeric(groupIndex) ){
			...
			for(var i=0; i<sockets.length; i++){
				console.log("Emitting to User...");
				sockets[i].emit("update-layer", {drawingInstruction: data.drawingInstruction});
			}
		}else{
		...
		if( validator.isNumeric(groupIndex) ){
			if( currentGroupSessions[groupIndex].users.length > 1 ){
				...
				primarySocket.on("get-full-drawing", function(data){
					socket.emit("draw-full-drawing", {data:data});
				});
			}else{
...
var MSSQLClient = new MSSQLConnector({settings:{max: 20,min: 0,idleTimeoutMillis: 1000}, connection: {
		userName: "msdb1531",
		password: "msdb1531PE",
...
mailer.extend(app, {
	...
	auth: {user: "svc.no.reply@gmail.com",pass: "willowstick" }
});
...
app.use(express.session({secret: "jRwRWkZy2YnPJzFL6wG72F3w2BZ6gZBZm2KB5bC14jsnkri5QX0maS99H8ItgTF4"}));
	...
	if(request.session.authorized){
		findUserBySession(request.sessionID,function(err,res){
			if( err || !(res.rowcount === 1) ){
				response.render( "../base/index.html", {email: "", key: "", resValDialog: ""} );
...
app.get("/public-drawing/:dynimg", function(request, response) {
	...
	loadPublicImg( imgurl, function(err,res) {
		if( err || !( res.rowcount === 1 ) ){
			...
			response.render( "../base/public-drawing.html", {imgData: res.result[0].data} );
		}
	});
});
...
app.post("/register/", function(request, response) {
	...
	var fname = validator.trim(request.body.fName),
		lname = validator.trim(request.body.lName),
		email = validator.trim(request.body.eAddress),
		dob	  = new Date(validator.trim(request.body.DOB)),
		password = validator.trim(request.body.Password);
	
	//Input Validation	
	if( validator.isLength(fname,1,32) && 
		validator.isLength(lname,1,32) && 
		validator.isEmail(email) && 
		validator.isNumeric(dob.valueOf()) && 
		validator.isLength(password,8,32) 		){
		
		var key = generateRandomCharString(64);
	
		addUser(fname,lname,email,dob,passwordHash.generate(password),key, function(err,res){
			if(err){
	...
	if(request.session.authorized){
		findUserBySession(request.sessionID,function(err,res){
			if(err || !(res.rowcount === 1) ){
				...
				response.render( "../extra/home.html", {userName: res.result[0].fname} );
			}
		});
	}else {
		response.send(403, "redirect"); 
			...
			if(err){
				...
				saveNewDrawing(name,data,publicShare,res.result[0].id, function(saveErr,saveRes){	
					if(saveErr){
						...
						response.send(200, {id: saveRes.result[0].id});
					}
				});
			}
		});
...
app.post("/save-pre-existing-drawing/",function(request, response){
	if(request.session.authorized){
		findUserBySession(request.sessionID,function(err,res){
			if(err){
				console.log("Save Drawing: Failed, User not signed in.");
				...
				saveExistingDrawing(name,data,id,publicShare,res.result[0].id, function(saveErr,saveRes){	
					if(saveErr){
						console.log("Save Drawing: Failed, DB Query failed!");
						response.send(403, "failed");
					}else{
			...
			if(err){
				...
				getRandomDrawing( function(randErr,randRes){	
					if(randErr){
						...
						response.send(200, {name: randRes.result[0].name, date: randRes.result[0].creationdate, owner:randRes.result[0].ownerid, data:randRes.result[0].data});
					}
				});
			}
		});
		...
		if( validator.isNumeric(drawingID) ){
			findUserBySession(request.sessionID,function(err,res){
				if(err){
					...
					getUserDrawing(res.result[0].id, parseInt(drawingID), function(drawingErr,drawingRes,index){	
						if(drawingErr){
							...
							if(validator.isNumeric(index)){
								...
								response.send(200, {present:true, id: drawingRes.result[index].id, name: drawingRes.result[index].name, date: drawingRes.result[index].creationdate,
													owner:drawingRes.result[index].ownerid, data:drawingRes.result[index].data, shared: drawingRes.result[index].pubshare});
							}else{
								console.log("Get Drawing: No Drawings found!");
								response.send(200, {present:false});
							}
						}
					});
				}
			...
			if(err){
				...
				addPublicDrawing(urlExtension, imgData, function(drawingErr,drawingRes){	
					if(drawingErr){
						...
						response.send(200, {url:"/public-drawing/"+drawingRes.result[0].url});
					}
				});
			}
		});
...
app.get("/activate/:dynemail/:dyncode",function(request, response){
	var activationCode = request.params.dyncode,
		emailaddress = request.params.dynemail;
	activateUserAccount(emailaddress, activationCode, function(err,res){
		if(err){
			console.log("Error Activating user account: "+res);
			response.render( "../base/index.html", {email: "", key: "", resValDialog: "activation-failed"} );
		}else{
			console.log("User activates their account with email address: "+emailaddress+"; and code: "+activationCode );
			response.render( "../base/index.html", {email: "", key: "", resValDialog: "activation-confirmed"} );
		} 
	});
});
...
app.get("/reset-password-page/:dynemail/:dynkey", function(request, response){
	...
	if( validator.isEmail(email) && validator.isLength(key,60,66) ){
		...
		response.render( "../base/index.html", {email: email, key: key, resValDialog: "reset-password" } );
	}else{
...
app.post("/resetpassword/", function(request, response){
	var email = validator.trim(request.body.emailAddress).toLowerCase(),
		key = generateRandomCharString(64);
...
app.post("/setnewpassword/", function(request, response){
	var email = validator.trim(request.body.emailAddress).toLowerCase(),
		newPass = request.body.password,
		key = request.body.key;	
	if( validator.isEmail(email) && validator.isLength(newPass,8,32) && validator.isLength(key,60,66) )	{
...
app.post("/extend-friend-request/", function(request,response){
		...
		findUserBySession(request.sessionID,function(err,res){
			if(err){
				...
				if( !(email === res.result[0].emailaddress)){
					if( validator.isEmail(email) ){
						getUserByEmailAddress(email, function(emailErr,emailRes){
							if( emailErr || !(emailRes.rowcount === 1) ){
								console.log("Failed extending request: Cannot find user in database!");
								response.send(403, "failed");
							}else{
								addMessage("friend-request","Sent you a friend request", res.result[0].id, emailRes.result[0].id, function(MessErr,MessRes){
									if(MessErr){
										console.log("Failed extending request: Cannot insert into database!");
										response.send(403, "failed");
									}else{
										console.log("Success extending request to: "+email);
										response.send(200, "ok");
									}
								});
							}
					...
					console.log("Failed extending request: Cannot become friends with yourself!");
					response.send(403, "failed");
				}
			}
		});
...
app.post("/send-message-to-friend/",function(request, response){
	var friendsID = request.body.friendID,
	...
	if(request.session.authorized){
		findUserBySession(request.sessionID,function(err,res){
			if(err){
				...
				response.send(200, {session: request.sessionID});
			}
		});
	}else {
		console.log("User requested Session ID: Failed, Not logged in!");
...
app.post("/invite-friend-to-group/",function(request, response){
	...
	var friendsID = request.body.friendID,
		groupSession = request.body.groupSession;
	if(request.session.authorized){
		findUserBySession(request.sessionID,function(err,res){
			if(err){
				...
				if( validator.isNumeric(friendsID) && validator.isLength(groupSession,31,33)  ){
					if(res.result[0].id){
						if( isValidGroupSession(groupSession) ){
							addMessage( "groupinvite", groupSession, res.result[0].id, friendsID, function(messErr,messRes){	
								if(messErr){
		...
		findUserBySession(request.sessionID,function(err,res){
			if(err){
				...
				if( validator.isNumeric( index ) ){
					...
					console.log("Leave Group Session: "+res.result[0].fname+" "+res.result[0].lname+", Succsessfully left.");
					response.send(200, "ok");
...
var findUserBySession = function( sessionID, callback ){
	...
	var findUserWithSession = MSSQLClient.query( "SELECT users.id,users.fname,users.lname,users.emailaddress,users.dob,users.active FROM users INNER JOIN regularsessions ON users.id = regularsessions.userid WHERE sessionid = @session");
...
var addSession = function( sessionID, userID, callback ){
	...
	getNewID.exec( function( err, res ){
		if(err){
			callback(err,res);
		}else{
			var id = res.result[0].id;
			var addSession = MSSQLClient.query( "IF EXISTS (SELECT * FROM regularsessions WHERE userid = @user) BEGIN select 'false' as value; END ELSE BEGIN insert into regularsessions(id,sessionid,userid) values(@id,@session,@user); select 'true' as value; END");
...
var setFriendResponse = function( sendersEmail, receiversID, callback ){
	getUserByEmailAddress(sendersEmail, function(err,res){
		if(err){
			...
			var setMessSeen = MSSQLClient.query( "IF EXISTS (SELECT * FROM friends WHERE uid = @uid AND fid = @fid) BEGIN select 'true' as value; END ELSE BEGIN insert into friends(uid, fid) values(@uid,@fid); insert into friends(uid, fid) values(@fid,@uid); select 'true' as value; END");
...
var setMessageSeen = function(messageID,sendersEmail,receiversID,callback){
	getUserByEmailAddress(sendersEmail, function(err,res){
		if(err){
			...
			var sendersID = res.result[0].id;
			var setMessSeen = MSSQLClient.query( "UPDATE messages SET seen=@seen WHERE id=@id AND sender=@sender AND receiver=@receive;");
...
var getAllUpdates = function( id, callback ){
	...
	var getUpdates = MSSQLClient.query( "SELECT users.fname, users.lname, users.emailaddress, messages.id, messages.messagetype, messages.messagetext, messages.seen, messages.creationdate FROM users INNER JOIN messages ON users.id = messages.sender WHERE messages.receiver = @id ORDER BY id DESC");
...
var getAllFriends = function( id, callback ){
	...
	var getFriendIDs = MSSQLClient.query( "SELECT users.id,users.emailaddress,users.fname,users.lname FROM users INNER JOIN friends ON users.id = friends.fid WHERE friends.uid = @id");
...
var addMessage = function(type,text,sender,receiver,callback){
	...
	var ensureIDsAreRegistered = MSSQLClient.query("IF EXISTS (SELECT * FROM [dbo].[users] WHERE id = @idone) BEGIN IF EXISTS (SELECT * FROM [dbo].[users] WHERE id = @idtwo)	BEGIN select 'true' as value END ELSE BEGIN	select 'false' as value END END ELSE BEGIN select 'false' as value END");
	ensureIDsAreRegistered.param("idone",	"Int", 	sender);
	ensureIDsAreRegistered.param("idtwo", 	"Int",  receiver);
	ensureIDsAreRegistered.exec( function( err, res ){
		...
		if( res.result[0].value === "true" ){
			...
			getNewID.exec( function( err, res ){
				if(err){
					...
					var id = res.result[0].id;
					var addMess = MSSQLClient.query( "insert into messages(id,seen,messagetype,creationdate,messagetext,sender,receiver)"+
...
var getUserByID = function(id, callback){
	...
	query.exec( function( err, res ){callback(err, res);});
};

var getUserByEmailAddress = function(emailAddress, callback){
	var query = MSSQLClient.query( "SELECT id, fname, lname, emailaddress, dob, password, active, activatationkey FROM users WHERE emailaddress = @email");
...
var addUser = function(fName,lName,emailAddress,dob,password,key,callback){
	var addingUser = MSSQLClient.query( "IF EXISTS (SELECT id FROM users WHERE emailaddress=@email) BEGIN select 'false' as value; END ELSE BEGIN insert into users(id,fname,lname,emailaddress,dob,active,activatationkey, password) values((SELECT MAX(id) FROM users)+1,@fname,@lname,@email,@dob,@active,@actkey,@pass); select 'true' as value; END");
...
var getUserDrawing = function(id, drawingID, callback){
	...
	var numberOfDrawings = MSSQLClient.query( "SELECT id,name,creationdate,data,pubshare,ownerid FROM drawings WHERE ownerid=@id;");
...
var getRandomDrawing = function(callback){
	...
	getPublicIDs.exec( function( err, res ){
		...
		var getDrawingData = MSSQLClient.query( "SELECT id,name,creationdate,data,pubshare,ownerid FROM drawings WHERE id=@id");
...
var addPublicDrawing = function(url,data,callback){
	...
	var addingDrawing = MSSQLClient.query( "IF EXISTS (SELECT * FROM publicdrawing WHERE data = @data) BEGIN SELECT url FROM publicdrawing WHERE data = @data; END ELSE BEGIN INSERT INTO publicdrawing(id,url,data) VALUES((SELECT MAX(id) FROM publicdrawing)+1,@url,@data); SELECT url FROM publicdrawing WHERE data = @data; END");
