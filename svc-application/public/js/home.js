var updatesShownCondition = false, updatesData = "", inGroupSession = false, vdrawData = "", myName = "";
		...
		if ( header.status === 200 ) {
			...
			$ ( "#banner_logo-img" ).click ( function( ) {
				...
				if ( inGroupSession ) {
					$.ajax ( {
					    type: "POST",
					    url: "/leave-group-session/",
					    data: {
						    session: $ ( "#group-session-session-id" ).html ( )
					    },
					    async: false
					} ).done ( function( ) {
	...
	$.post ( "/get-my-drawing/", {
		...
		if ( data.present ) {
			...
			$ ( "#view-my-drawings-delete-button" ).click ( function( ) {
				...
				$.post ( "/delete-my-drawing/", {
					id: vdrawData.id
				} ).done ( function( ) {

				} ).fail ( function( ) {

					alert ( "Failed" );/* Reimplement */
				} );
				canContext.clearRect ( 0, 0, 700, 700 );
				canContext.rect ( 0, 0, 700, 700 );
				canContext.fillStyle = "#FFFFFF";
...
var openFriendRequestResponseDialog = function( index ) {
	...
	var id = updatesData [ index ].id, name = updatesData [ index ].fname + " " + updatesData [ index ].lname + " ("
	        + updatesData [ index ].emailaddress + ")", email = updatesData [ index ].emailaddress, message = updatesData [ index ].messagetext;
	openUpdatesMenu ( );
	$.post ( "/updates-set-seen/", {
	    id: id,
	    email: email
	} );
...
var joinGroupSession = function( index ) {
	...
	var id = updatesData [ index ].id, name = updatesData [ index ].fname + " " + updatesData [ index ].lname + " ("
	        + updatesData [ index ].emailaddress + ")", email = updatesData [ index ].emailaddress, session = updatesData [ index ].messagetext;
	openUpdatesMenu ( );
	$.post ( "/updates-set-seen/", {
	    id: id,
	    email: email
	                ...
	                function( data, status, header ) {
		                ...
		                if ( $ ( "#res-number-of-updates" ).html ( ) === "" ) {
			                ...
			                for ( var i = 0; i < data.length; i++ ) {
				                if ( !( data [ i ].seen ) ) {
					                first = false;
					                $ ( "#banner_updates-button" ).effect ( "highlight", {
						                color: "rgba(255,0,0,1.0);"
					                }, 500 );
					                setTimeout ( function( ) {

						                $ ( "#banner_updates-button" ).css ( "background-color", "rgba(255,0,0,0.2)" );
					                }, 550 );
				                }
			                }
		                }else {
		                ...
		                for ( var i = 0; i < data.length; i++ ) {
			                ...
			                if ( data [ i ].messagetype === "friend-request" ) {
				                html += "<div onclick=\"openFriendRequestResponseDialog(" + i + ")\" class=\""
				                        + classValue + "\">" + data [ i ].fname + " " + data [ i ].lname + " ("
				                        + data [ i ].emailaddress + ")<br><div class='updates-text-separator' ></div>"
				                        + data [ i ].messagetext + "</div>";
			                }else if ( data [ i ].messagetype === "message" ) {
				                html += "<div onclick=\"openMessageDialog(" + i + ")\" class=\"" + classValue + "\">"
				                        + data [ i ].fname + " " + data [ i ].lname + " (" + data [ i ].emailaddress
				                        + ")<br><div class='updates-text-separator' ></div>Sent you a message</div>";
			                }else if ( data [ i ].messagetype === "groupinvite" ) {
				                html += "<div onclick=\"joinGroupSession("
				                        + i
				                        + ")\" class=\""
				                        + classValue
				                        + "\">"
				                        ...
				                        + " ("
				                        + data [ i ].emailaddress
				                        + ")<br><div class='updates-text-separator' ></div>Invited you to join a group session</div>";
			                }
		                }
...
var messageAFriend = function( ) {
	...
	$.post ( "/get-friends-list/", {} ).done (
	        function( res, status, header ) {

		        var selectorText = "<option value='-'>Select A Friend</option>";
		        var friends = header.responseJSON;
		        for ( var i = 0; i < friends.length; i++ ) {
			        selectorText += "<option value='" + friends [ i ].id + "'>" + friends [ i ].fname + " "
			                + friends [ i ].lname + " (" + friends [ i ].emailaddress + ") </option>";
		        }
		        ...
		        $ ( "#dialog-window-submit-button" ).click ( function( ) {
			        ...
			        if ( $ ( "#message-to-friend-friend-selection" ).find ( ":selected" ).val ( ) === "-" ) {
				        $ ( "#message-to-friend-friend-selection" ).effect ( "highlight", {
					        color: "rgba(255,0,0,0.1);"
				        }, 500 );
			        }else {
				        $.post ( "/send-message-to-friend/", {
				            friendID: $ ( "#message-to-friend-friend-selection" ).find ( ":selected" ).val ( ),
				            message: $ ( "#message-to-friend-text-area" ).val ( )
...
var extendFriendRequest = function( ) {
	...
	$ ( "#dialog-window-submit-button" ).click ( function( ) {
		...
		if ( isValidEmailAddress ( $.trim ( $ ( "#friend-request-friend-email" ).val ( ) ) ) ) {
			$.post ( "/extend-friend-request/", {
				friendemail: $ ( "#friend-request-friend-email" ).val ( )
			} ).done ( function( data, st, resHead ) {
			...
			$ ( "#friend-request-friend-email" ).effect ( "highlight", {
				color: "#FF0000"
			}, 500 );
		}
	} );
	...
	$.post ( "/get-friends-list/", {} ).done (
	        function( res, status, header ) {
		        ...
		        for ( var i = 0; i < friends.length; i++ ) {
			        ...
			        friendsHTML += "<td class=\"invite-friends-selector-text\">" + friends [ i ].fname + " "
			                + friends [ i ].lname + "</td><td><input value=\"" + friends [ i ].id
			                + "\"id=\"select-friends-" + i + "\" type=\"checkbox\" name=\"select-friends\"></td>";
			        if ( ( i & 1 ) ) {
				        friendsHTML += "</tr>";
			        }
		        }
		...
		$.post ( "/start-group-session/", {} ).done ( function( sessionID ) {
			...
			for ( var i = 0; i < numOfFriends; i++ ) {
				if ( $ ( "#select-friends-" + i ).is ( ":checked" ) ) {
					$.post ( "/invite-friend-to-group/", {
					    friendID: $ ( "#select-friends-" + i ).val ( ),
