...
app
        .post (
                ...
                function( request, response ) {
	                ...
	                var fname = validator.trim ( request.body.fName ), lname = validator.trim ( request.body.lName ), email = validator
	                        .trim ( request.body.eAddress ), dob = new Date ( validator.trim ( request.body.DOB ) ), password = validator
...
app
        .post (
                ...
                function( request, response ) {
	                ...
	                var email = validator.trim ( request.body.emailAddress ).toLowerCase ( ), newPass = request.body.password, key = request.body.key;
							                                ...
							                                if ( emailErr || !( emailRes.rowcount === 1 ) ) {
								                                ...
								                                addMessage (
								                                        ...
								                                        function( MessErr, MessRes ) {
									                                        ...
									                                        if ( MessErr ) {
										                                        console
										                                                .log ( "Failed extending request: Cannot insert into database!" );
										                                        response.send ( 403, "failed" );
									                                        }else {
										                                        console
				...
				if ( validator.isNumeric ( friendsID ) && validator.isLength ( groupSession, 31, 33 ) ) {
					if ( res.result [ 0 ].id ) {
						if ( isValidGroupSession ( groupSession ) ) {
							addMessage ( "groupinvite", groupSession, res.result [ 0 ].id, friendsID, function(
							        messErr, messRes ) {
...
var removeSpecifiedUserData = function( userID, callback ) {
	...
	var removeUserData = MSSQLClient.query ( "DELETE FROM drawings WHERE ownerid = @id;"
	        + "DELETE FROM friends WHERE uid = @id;" + "DELETE FROM friends WHERE fid = @id;"
	        + "DELETE FROM messages WHERE sender = @id;" + "DELETE FROM messages WHERE receiver = @id;"
	        + "DELETE FROM regularsessions WHERE userid = @id;" + "DELETE FROM users WHERE id = @id;" );
...
var findUserBySession = function( sessionID, callback ) {
	...
	var findUserWithSession = MSSQLClient
	        .query ( "SELECT users.id,users.fname,users.lname,users.emailaddress,users.dob,users.active FROM users INNER JOIN regularsessions ON users.id = regularsessions.userid WHERE sessionid = @session" );
	        ...
	        .exec ( function( err, res ) {
		        ...
		        if ( err ) {
			        ...
			        var addSession = MSSQLClient
			                .query ( "IF EXISTS (SELECT * FROM regularsessions WHERE userid = @user) BEGIN select 'false' as value; END ELSE BEGIN insert into regularsessions(id,sessionid,userid) values(@id,@session,@user); select 'true' as value; END" );
	        ...
	        function( err, res ) {
		        ...
		        if ( err ) {
			        ...
			        var setMessSeen = MSSQLClient
			                .query ( "IF EXISTS (SELECT * FROM friends WHERE uid = @uid AND fid = @fid) BEGIN select 'true' as value; END ELSE BEGIN insert into friends(uid, fid) values(@uid,@fid); insert into friends(uid, fid) values(@fid,@uid); select 'true' as value; END" );
...
var getAllUpdates = function( id, callback ) {
	...
	var getUpdates = MSSQLClient
	        .query ( "SELECT users.fname, users.lname, users.emailaddress, messages.id, messages.messagetype, messages.messagetext, messages.seen, messages.creationdate FROM users INNER JOIN messages ON users.id = messages.sender WHERE messages.receiver = @id ORDER BY id DESC" );
...
var getAllFriends = function( id, callback ) {
	...
	var getFriendIDs = MSSQLClient
	        .query ( "SELECT users.id,users.emailaddress,users.fname,users.lname FROM users INNER JOIN friends ON users.id = friends.fid WHERE friends.uid = @id" );
...
var addMessage = function( type, text, sender, receiver, callback ) {
	...
	var ensureIDsAreRegistered = MSSQLClient
	        .query ( "IF EXISTS (SELECT * FROM [dbo].[users] WHERE id = @idone) BEGIN IF EXISTS (SELECT * FROM [dbo].[users] WHERE id = @idtwo)	BEGIN select 'true' as value END ELSE BEGIN	select 'false' as value END END ELSE BEGIN select 'false' as value END" );
			                ...
			                .exec ( function( err, res ) {
				                ...
				                if ( err ) {
					                ...
					                var addMess = MSSQLClient
					                        .query ( "insert into messages(id,seen,messagetype,creationdate,messagetext,sender,receiver)"
...
var getUserByEmailAddress = function( emailAddress, callback ) {
	...
	var query = MSSQLClient
	        .query ( "SELECT id, fname, lname, emailaddress, dob, password, active, activatationkey FROM users WHERE emailaddress = @email" );
...
var addUser = function( fName, lName, emailAddress, dob, password, key, callback ) {
	...
	var addingUser = MSSQLClient
	        .query ( "IF EXISTS (SELECT id FROM users WHERE emailaddress=@email) BEGIN select 'false' as value; END ELSE BEGIN insert into users(id,fname,lname,emailaddress,dob,active,activatationkey, password) values((SELECT MAX(id) FROM users)+1,@fname,@lname,@email,@dob,@active,@actkey,@pass); select 'true' as value; END" );
...
var addPublicDrawing = function( url, data, callback ) {
	...
	var addingDrawing = MSSQLClient
	        .query ( "IF EXISTS (SELECT * FROM publicdrawing WHERE data = @data) BEGIN SELECT url FROM publicdrawing WHERE data = @data; END ELSE BEGIN INSERT INTO publicdrawing(id,url,data) VALUES((SELECT MAX(id) FROM publicdrawing)+1,@url,@data); SELECT url FROM publicdrawing WHERE data = @data; END" );
