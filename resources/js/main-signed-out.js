...
$(document).ready(function(){
	...
	$( "#replaceable-small" ).fadeTo( 400, 0.0, function(){
		...
		setTimeout(function(){ $( "#replaceable-small" ).fadeTo( 400, 1.0 ) }, 400);
	});
	...
	setTimeout(function(){
		...
		if( $("#dialog-to-open").html() === "activation-confirmed" ){
			...
			$(".dialog-window" ).html( $( "#dialog-window-registration-active-failed" ).html() );	
			$(".dialog-window").css("visibility", "visible");
			$(".dialog-window").css("opacity", "0");
			$(".dialog-window" ).fadeTo( 400, 1.0 );
			$( ".dialog-window-backdrop" ).click(function() {
			...
			$(".dialog-window" ).html( $( "#dialog-window-reset-password-original" ).html() );	
			$(".dialog-window").css("visibility", "visible");
			$(".dialog-window").css("opacity", "0");
			$(".dialog-window" ).fadeTo( 400, 1.0 );
			$( "#dialog-window-submit-button" ).click(function() {
				...
				if( ($("#newPassword").val() === $("#cPassword").val()) && ($.trim( $("#newPassword").val().length ) > 7) ){
					...
					$.post( "/setnewpassword/", {password: $("#newPassword").val(), key: keyval, emailAddress: email}).done(function() {
						$(".dialog-window" ).html( $( "#dialog-window-reset-password-success" ).html() );	
						$(".dialog-window").css("visibility", "visible");
						$(".dialog-window").css("opacity", "0");
						$(".dialog-window" ).fadeTo( 400, 1.0 );
						$( ".dialog-window-backdrop" ).click(function() {
						...
						$(".dialog-window" ).html( $( "#dialog-window-reset-password-failed" ).html() );	
						$(".dialog-window").css("visibility", "visible");
						$(".dialog-window").css("opacity", "0");
						$(".dialog-window" ).fadeTo( 400, 1.0 );
						$( ".dialog-window-backdrop" ).click(function() {
...
var forgottenPassword = function (){
	...
	$( "#dialog-window-submit-button" ).click(function() {
		if( isValidEmailAddress($.trim($("#reset-pass-emailAddress").val())) ){
			$.post( "/resetpassword/", {emailAddress: $("#reset-pass-emailAddress").val()
			}).done(function() {
			...
			$("#reset-pass-emailAddress").effect("highlight", {color:"#FF0000"}, 500); 
		}
	});
};
...
var signIn = function (){
	...
	$.post( "/sign-in/", {emailAddress: $("#emailAddress").val(), password: $("#password").val()})
	.done(function() {
		...
		$("#sign-in-table_failed-text").effect("highlight", {color:"#FF0000"}, 500); 
	});
};
...
var signUp = function (){
	...
	if(!($.trim($("#fName").val().length) > 0)){
		$("#fName").effect("highlight", {color:"rgba(255,0,0,0.1);"}, 500);
		proceed = false;
	}
	
	if(!($.trim($("#lName").val().length) > 0)){
		$("#lName").effect("highlight", {color:"rgba(255,0,0,0.1);"}, 500);
		proceed = false;
	}	
	
	if(!isValidEmailAddress($.trim($("#eAddress").val()))){
		$("#eAddress").effect("highlight", {color:"rgba(255,0,0,0.1);"}, 500);
		proceed = false;
	}	
	
	if(!$.isNumeric(new Date($("#DOB").val()).getTime())){
		$("#DOB").effect("highlight", {color:"rgba(255,0,0,0.1);"}, 500);
		proceed = false;
	}
		
	if(!($("#oPassword").val() === $("#cPassword").val()) || !($.trim( $("#oPassword").val().length ) > 7) ){
		$("#oPassword").effect("highlight", {color:"rgba(255,0,0,0.1);"}, 500);
		$("#cPassword").effect("highlight", {color:"rgba(255,0,0,0.1);"}, 500); 	
		proceed = false;		
	}
				
	if(proceed){
		$.post( "/register/", {	fName: $("#fName").val(), 
								lName: $("#lName").val(),
		...
		.fail(function() {
			...
			$("#register-table_failed-text").effect("highlight", {color:"rgba(255,0,0,0.75);"}, 500); 	
		});					
	}else{
		allowRegistration = true;
	}			
