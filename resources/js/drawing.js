...
var attachEvents = function(){ 
	...
	document.onmousedown = function(event) { 
		...
		if(	sx>=0 && sx<=700 && sy>=0 && sy<=700 ){
			...
			if( currShapeSelected === "rectangle" ){
				...
				currShapeHandle.initialise($("#line-width").val(),$("#color-filled").is(":checked"),hex2rgb($("#color-color").val(),($("#color-opacity").val()/100)),sx,sy);
				...
				currShapeHandle.initialise($("#line-width").val(),hex2rgb($("#color-color").val(),($("#color-opacity").val()/100)),sx,sy);
				...
				currShapeHandle.initialise($("#line-width").val(),$("#font-selector").find(":selected").val(),hex2rgb($("#color-color").val(),($("#color-opacity").val()/100)),sx,sy);
				if($("#text-value").val() === ""){
	...
	document.onmouseup = function(event) { 
		var x = event.clientX - ( $("#canvas-holder").offset().left + 10 ),
			y = event.clientY - ( $("#canvas-holder").offset().top + 10 );
	...
	document.onmousemove = function(event) {
		...
		if( x>=0 &&	x<=700 && y>=0 && y<=700 ){
			if( currShapeSelected === "stext" ){
				...
				drawText(phantomContext,x,y,text,$("#line-width").val(),$("#font-selector").find(":selected").val(),hex2rgb($("#color-color").val(),($("#color-opacity").val()/100)));
			}else if( currShapeSelected === "cerazer" ){
				...
				clearCanvas(phantomContext);
				drawSimpleCircle(phantomContext,x,y,($("#line-width").val()-$("#line-width").val()/20)/2,$("#line-width").val()/20,false,hex2rgb(erazerCircleColour,1));
				...
				clearCanvas(phantomContext);
				drawRectangle(phantomContext,x,y,x+parseInt($("#line-width").val()),y+parseInt($("#line-width").val()),$("#line-width").val()/20,false,hex2rgb(erazerCircleColour,1));
			}
		}
		...
		if(	x>=0 &&	x<=700 && y>=0 && y<=700 && isMouseDown){
			...
			if( currShapeSelected === "rectangle" ){
				clearCanvas(phantomContext);
				drawRectangle(phantomContext,sx,sy,x,y, $("#line-width").val(), $("#color-filled").is(":checked"),hex2rgb($("#color-color").val(),($("#color-opacity").val()/100)));
			}else if( currShapeSelected === "triangle" ){
				clearCanvas(phantomContext);
				drawTriangle(phantomContext,sx,sy,x,y, $("#line-width").val(), $("#color-filled").is(":checked"),hex2rgb($("#color-color").val(),($("#color-opacity").val()/100)));
			}else if( currShapeSelected === "arrow" ){
				clearCanvas(phantomContext);
				drawArrow(phantomContext,sx,sy,x,y, $("#line-width").val(), $("#color-filled").is(":checked"),hex2rgb($("#color-color").val(),($("#color-opacity").val()/100)));
			}else if( currShapeSelected === "circle" ){
				clearCanvas(phantomContext);
				drawCircle(phantomContext,sx,sy,x,y, $("#line-width").val(), $("#color-filled").is(":checked"),hex2rgb($("#color-color").val(),($("#color-opacity").val()/100)));
			}else if( currShapeSelected === "straightline" ){
				clearCanvas(phantomContext);
				drawStraightLine(phantomContext,sx,sy,x,y, $("#line-width").val(),hex2rgb($("#color-color").val(),($("#color-opacity").val()/100)));
			}else if( currShapeSelected === "quadraticcurve" ){
				if( quadStage === 1 ){
					clearCanvas(phantomContext);
					drawStraightLine(phantomContext,sx,sy,x,y,$("#line-width").val(),hex2rgb($("#color-color").val(),($("#color-opacity").val()/100)));
					...
					clearCanvas(phantomContext);
					drawQuadraticCurve(phantomContext,currShapeHandle.sx,currShapeHandle.sy,x,y,currShapeHandle.fx,currShapeHandle.fy,$("#line-width").val(),hex2rgb($("#color-color").val(),($("#color-opacity").val()/100)));
				}
			}else if( currShapeSelected === "berziercurved" ){
				if( berzierStage === 1 ){
					clearCanvas(phantomContext);
					...
					clearCanvas(phantomContext);
					drawBerzierCurve(phantomContext,currShapeHandle.sx,currShapeHandle.sy,currShapeHandle.c1x,currShapeHandle.c1y,x,y,currShapeHandle.fx,currShapeHandle.fy,$("#line-width").val(),hex2rgb($("#color-color").val(),($("#color-opacity").val()/100)));
				}
			}else if( currShapeSelected === "pencil" ){
				clearCanvas(phantomContext);
				drawPencil(phantomContext,currShapeHandle,x,y,$("#line-width").val(),hex2rgb($("#color-color").val(),($("#color-opacity").val()/100)));
			}else if( currShapeSelected === "cerazer" ){
				clearCanvas(phantomContext);
...
var saveToServer = function(){
	if( currentDrawing.getJSON() === undefined ){
		$("#save-drawing").effect("highlight", {color:"#00FF00"}, 500); 
	}else{		
		if(currentDrawing.id){
			$.post( "/save-pre-existing-drawing/", {data: JSON.stringify(currentDrawing.getJSON()), name: currentDrawing.getName(), id:currentDrawing.id, publicShare: $("#share-publicly-allowed").is(":checked") }).done(function() {
				$("#save-drawing").effect("highlight", {color:"#00FF00"}, 500); 
			}).fail(function() {
				$("#save-drawing").effect("highlight", {color:"#FF0000"}, 500); 
			});
		}else{
			$.post( "/save-drawing/", {data: JSON.stringify(currentDrawing.getJSON()), name: currentDrawing.getName(), publicShare: $("#share-publicly-allowed").is(":checked") }).done(function(data) {
...
var clearDrawingData = function(){
	...
	if(isGroupSession){
		connection.emit("group-clear-canvas", {groupSession: groupSessionID, sessionID: mySessionID});
...
var addToolSelectionListeners = function(){
	...
	$( "#shape-circle" ).click(function() {		
		...
		$(this).effect("highlight", {color:"#ffffff"}, 500); 
...
var drawArrow = function(canHandle,sX,sY,fX,fY,outlineWidth,filled,lColor){
	...
	canHandle.moveTo(sX, sY);
	canHandle.lineTo(fX, fY);
	canHandle.lineTo(fX-headlen*Math.cos(angle-Math.PI/6),fY-headlen*Math.sin(angle-Math.PI/6));
...
var drawCircle = function(canHandle,sX,sY,fX,fY,outlineWidth,filled,lColor){
	...
	var inner = Math.pow( (fX-sX), 2) + Math.pow( (fY-sY), 2 ),
		radius = Math.sqrt(inner)/2,
		x = (sX+fX)/2,
		y = (sY+fY)/2;
...
var drawingInstructions = function(){
	...
	this.name;	this.drawingSequence;
	this.id;	this.publicShare;
...
var rectangleShape = function(){
	...
	this.sx;	this.sy;	this.fx;
	this.fy;	this.width;	this.filled;
	this.colour;this.shape;	this.visible;
	
	this.initialise = function(w,f,c,x,y){
		this.width = w;		this.filled = f;	this.visible = true;
		this.colour = c;	this.sx = x;
		this.sy = y;		this.shape = "rectangle";
	};
	this.setEndPoints = function(x,y){
		this.fx = x;		this.fy = y;
	};
	this.getJSON = function(){
		return {"shape": this.shape, "sX": this.sx, "sY": this.sy, "fX": this.fx, "fY": this.fy, "lWidth": this.width, "filled": this.filled, "colour": this.colour, "visible": this.visible };
	};
};
...
var textShape = function(){
	...
	this.sX;		this.sY;		this.text;	
	this.size;		this.font;	this.lColor;
	this.shape;		this.visible;
	
	this.initialise = function(w,f,c,x,y){
		this.size = w;		this.font = f;	this.shape = "stext";	this.visible = true;
		this.lColor = c;	this.sX = x;	this.sY = y;
	};
	this.setText = function(t){
		this.text = t;
	};
	this.getJSON = function(){
		return {"shape": this.shape, "sX": this.sX, "sY": this.sY, "lWidth": this.size, "text": this.text, "font": this.font, "colour": this.lColor, "visible": this.visible};
	};
};
...
var groupChatMessageShape = function(){
	...
	this.initialise = function(name,text){
		this.shape = "groupmessage";	this.text = text; 
		this.senderName = name;			this.visible = true;
	};
	
	this.getJSON = function(){
		return {"shape": this.shape, "text": this.text, "sender": this.senderName, "visible": this.visible};
	};
};
...
var straightLineShape = function(){
	...
	this.sx;	this.sy;	this.fx;
	this.fy;	this.width;	this.colour;
	this.shape; this.visible;
	
	this.initialise = function(w,c,x,y){
		this.width = w;		this.colour = c;				this.sx = x;
		this.sy = y;		this.shape = "straightline";	this.visible = true;
	};
	this.setEndPoints = function(x,y){
		this.fx = x;		this.fy = y;
	};
	this.getJSON = function(){
		return {"shape": this.shape, "sX": this.sx, "sY": this.sy, "fX": this.fx, "fY": this.fy, "lWidth": this.width, "colour": this.colour, "visible": this.visible};
	};
};
...
var quadraticCurveShape = function(){
	...
	this.sx;	this.sy;	this.fx;
	this.fy;	this.width;	this.colour;
	this.shape; this.cx; 	this.cy;
	this.visible;
	
	this.initialise = function(w,c,x,y){
		this.width = w;		this.colour = c;				this.sx = x;
		this.sy = y;		this.shape = "quadraticcurve";	this.visible = true;
	};
	...
	this.getJSON = function(){
		return {"shape":this.shape,"sX":this.sx,"sY":this.sy,"cx":this.cx,"cy":this.cy,"fX":this.fx,"fY":this.fy,"lWidth":this.width,"colour":this.colour, "visible": this.visible};
	};
};
...
var berzierCurveShape = function(){
	...
	this.sx;	this.sy;	this.fx;
	this.fy;	this.width;	this.colour;		this.visible;
	this.shape; this.c1x; 	this.c1y; this.c2x; this.c2y; 
	
	this.initialise = function(w,c,x,y){
		this.width = w;		this.colour = c;				this.sx = x;
		this.sy = y;		this.shape = "berziercurved";	this.visible = true;
	};
	...
	this.getJSON = function(){
		return {"shape":this.shape,"sX":this.sx,"sY":this.sy,"c1x":this.c1x,"c1y":this.c1y,"c2x":this.c2x,"c2y":this.c2y,"fX":this.fx,"fY":this.fy,"lWidth":this.width,"colour":this.colour, "visible": this.visible};
	};
};

var pencilShape = function(){
	
	this.positions;	this.width;	this.visible;
	this.colour;	this.shape;
	
	this.initialise = function(w,c,x,y){
		this.width = w;				this.colour = c;	this.shape = "pencil";
		this.positions = new Array;	this.positions.push({"x":x,"y":y});
		this.visible = true;
	};
	
	this.getJSON = function(){
		return {"shape": this.shape, "positions": this.positions, "lWidth": this.width, "colour": this.colour, "visible": this.visible};
	};
};

var erazeSquareShape = function(){
	
	this.sX;	this.sY;	this.visible;
	this.width;	this.shape;
	
	this.initialise = function(w,x,y){
		this.width = w;		this.shape = "serazer";		this.visible = true;
		this.sX = x; 		this.sY = y;				
	};
	
	this.getJSON = function(){
		return {"shape": this.shape, "sX": this.sX, "sY": this.sY, "lWidth": this.width, "visible": this.visible};
	};
};

var erazeCircleShape = function(){
	
	this.positions;	this.width;	
	this.shape;		this.visible;
	
	this.initialise = function(w,x,y){
		this.width = w;						this.shape = "cerazer";
		this.positions = new Array;			this.visible = true;
		this.positions.push({"x":x,"y":y});
	};
	
	this.getJSON = function(){
		return {"shape": this.shape, "positions": this.positions, "lWidth": this.width, "visible": this.visible};
	};
};
...
var recordShape = function(cSH){
	...
	if( cshD.shape === "rectangle" ){
		...
		drawBerzierCurve(canContext,cshD.sX,cshD.sY,cshD.c1x,cshD.c1y,cshD.c2x,cshD.c2y,cshD.fX,cshD.fY,cshD.lWidth,cshD.colour);
	}else if( cshD.shape === "pencil" ){
		drawPencil(canContext,cSH,cSH.positions[cSH.positions.length-1].x+1,cSH.positions[cSH.positions.length-1].y+1,cshD.lWidth,cshD.colour);
	}else if( cshD.shape === "serazer" ){
		erazeSquare(canContext,cSH.sX,cSH.sY,cshD.lWidth);
	}else if( cshD.shape === "cerazer" ){
		erazeCircle(canContext,cSH,cSH.positions[cSH.positions.length-1].x+1,cSH.positions[cSH.positions.length-1].y+1,cshD.lWidth);
		...
		$("#messages-content-view").append( "<div class=\"messages-message-text\">"+cshD.sender+": "+cshD.text+"</div>" );
		if( !scrollRecent ){
			scrollRecent = true;
	...
	if(isGroupSession){
		connection.emit("send-group-message", {groupSession: groupSessionID, sessionID: mySessionID, drawingInstruction: cshD});
	}
};
	...
	for(i=0; i<instructions.length; i++){
		...
		if( curr.visible ){
			if( curr.shape === "rectangle" ){
				...
				drawQuadraticCurve(canvasHandle,curr.sX,curr.sY,curr.cx,curr.cy,curr.fX,curr.fY,curr.lWidth,curr.colour);
			}else if( curr.shape === "berziercurved"){
				drawBerzierCurve(canvasHandle,curr.sX,curr.sY,curr.c1x,curr.c1y,curr.c2x,curr.c2y,curr.fX,curr.fY,curr.lWidth,curr.colour);
			}else if( curr.shape === "pencil" ){
				drawPencil(canvasHandle,curr,curr.positions[curr.positions.length-1].x,curr.positions[curr.positions.length-1].y,curr.lWidth,curr.colour);
			}else if( curr.shape === "serazer" ){
				erazeSquare(canvasHandle,curr.sX,curr.sY,curr.lWidth);
			}else if( curr.shape === "cerazer" ){
				erazeCircle(canvasHandle,curr,curr.positions[curr.positions.length-1].x,curr.positions[curr.positions.length-1].y,curr.lWidth);
				...
				if(isGroupSession){
					$("#messages-content-view").append( "<div class=\"messages-message-text\">"+curr.sender+": "+curr.text+"</div>" );
					if( !scrollRecent ){
						scrollRecent = true;
						$("#messages-main-view").animate({ scrollTop: $("#messages-content-view").height() }, 500 );
						setTimeout(function(){ scrollRecent = false; }, 500);
...
var initPrivateSession = function(){
	...
	$( "#shape-selector-accordion" ).accordion({ fillSpace: true, heightStyle: "fill" });
	$( "#colour-selector-accordion" ).accordion({ fillSpace: true, heightStyle: "fill" });
	...
	for(var i=0; i<instructions.length; i++){
		...
		if( !( instructions[i].shape === "groupmessage" ) ){
			...
			$("#edit-layers-main-shape-container").append(
				"<div class=\"edit-layers-shape-container\">"
				+	"<canvas id=\"canvas-shape-layer-"+i+"\" class=\"edit-layers-canvas\" width=\"700\" height=\"700\" />"
				+	"<div class=\"edit-layers-option-container\">"
				+		"<div class=\"edit-layers-shape-name\">"+shapeName+"</div><div class=\"edit-layers-options-sub-container\">"
				+			"<table>"
				+				"<tr>"
				+					"<td class=\"td-text-light\" >Visible:</td>"
				+					"<td class=\"td-input\"><input id=\"shape-visible-"+i+"\" type=\"checkbox\" onclick=\"changeVisibilityStatus("+i+")\" name=\"shape-filled\" "+visibleval+" ></td>"
				+				"</tr>"
				+				"<tr>"
				+					"<td class=\"td-text-light\" >Shape Filled:</td>"
				+					"<td class=\"td-input\"><input id=\"shape-filled-"+i+"\" type=\"checkbox\" onclick=\"changeFilledStatus("+i+")\" name=\"shape-filled\" "+filledval+" ></td>"
				+				"</tr>	"
				+				"<tr class=\"tr-separator\" >"
				+					"<td class=\"td-text-light\">Colour:</td>"
				+					"<td class=\"td-input\"><input id=\"color-color-"+i+"\" type=\"color\" name=\"color-chooser\" value=\""+colour+"\" onchange=\"changeColourStatus("+i+")\" class=\"colour-selector-input-width\"></td>"
				+				"</tr>	"
				+				"<tr class=\"tr-separator\" >"
				+					"<td class=\"td-text-light\">Opacity:</td> "
				+					"<td class=\"td-input\"><input id=\"color-opacity-"+i+"\" type=\"range\" value=\""+opacity+"\" onchange=\"changeColourStatus("+i+")\" name=\"opacity-chooser\" class=\"colour-selector-input-width\"></td>"
				+				"</tr>"
				+				"<tr class=\"tr-separator\" >"
				+					"<td class=\"td-text-light\">Line Width:</td>"
				+					"<td class=\"td-input\"><input id=\"line-width-"+i+"\" type=\"number\" value=\""+width+"\" onchange=\"changeWidthStatus("+i+")\" name=\"line-width\" class=\"colour-selector-input-width\"></td>"
				+				"</tr>"
				+			"</table>"
				+		"</div>"
				+	"</div>" 
...
var updateLayersGroup = function(id,typ,val){
	connection.emit("group-update-layers", {groupSession: groupSessionID, sessionID: mySessionID, drawingInstruction: {elemID:id, type: typ, value: val }});
};
